...
module ArrayLib
  ...
  def to_indices_hash(arr)
    ...
    arr.each_with_object({}) do |e, hash|
      hash[hash.size] = e
    end
  end
  ...
  def group_aoh_by_key(arr, key, parallel_array = nil)
    ...
    arr.each_with_index do |h, i|
      value = h[key]
      group[value] = [] unless group.has_key?(value)
  ...
  def eval_map(arr, meth_string)
    if /\./.match?(meth_string)
      arr.map { |_e| eval "#{meth_string}(e)" }
    else
  ...
  def split_by(arr, divider)
    arr.each_with_object([[]]) do |element, results|
